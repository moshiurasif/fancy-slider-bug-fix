...
const showImages = (images) => {
  ...
  loadingSpinner(); // hide spinner after loading data
}

const getImages = (query) => {
  loadingSpinner(); // show spinner on data load time
...
const createSlider = () => {
  ...
  if(durationInput < 0){ 
    alert('Duration time cannot be negative! Give valid duration input.');
    return;
  }
...
// extra features play / pause function
const playPause = () => {
  if(pause === false){
...
// extra features play / pause button event handler
document.getElementById('pause-play-btn').addEventListener('click', event => {
  const playPauseButton = event.target.innerText;
  if(playPauseButton === 'Pause'){
    pause = true;
...
// extra features Loading spinner show / hide
